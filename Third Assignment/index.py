...
class SPIMIIndexer(Indexer):
    ...
    def build_index(self, reader, tokenizer, index_output_dir):
        ...
        while reader.has_documents_to_read():
            ...
            for document_id in documents:
                ...
                for count, term in enumerate(terms):
                    ...
                    self.get_index().add_term(term, {document_id: terms_frequency[term]}, {document_id: {count}})
        ...
        # Save documents info
        self.documents_info.save_to_disk(f"{index_output_dir}/documents_info.jsonl")

        # Save tokenizer info
        self.save_tokenizer_info(index_output_dir)
    ...
    def merge_partial_indexes(self, partial_indexes_output_dir, final_index_output_dir):
        ...
        # Structure to store the postings lists of smallest term of each partial index
        temp_index = {}

        # Structure to store the positions of smallest term of each partial index
        temp_pos = {}
        ...
        for path in os.scandir(partial_indexes_output_dir):
            if path.is_file():
                ...
                # Temporarily store term, postings list and positions
                temp_index[partial_id] = {term: postings}
                temp_pos[partial_id] = {term: positions}
        ...
        while not merge_completed:
            ...
            # Build a new positions list with the current smallest term
            new_positions = {}
            ...
            for partial_id in temp_index:
                for term in temp_index[partial_id]:
                    if term == smallest_term:
                        ...
                        # positions = {'1854001': {0}, '1854092': {0}, '1854676': {4, 10}, '1854746': {5, 30}}
                        postings = temp_index[partial_id][term]
                        positions = temp_pos[partial_id][term]

                        # Note: Postings list and positions have the same document IDs
                        for doc_id in postings:
                            new_postings[doc_id] = postings[doc_id]
                            new_positions[doc_id] = positions[doc_id]

            # Add term and postings list to the final index
            self.get_index().add_term(smallest_term, new_postings, new_positions)
            ...
            for partial_id in smallest_partial_ids:
                ...
                if line != "":
                    ...
                    # Store new smallest term, postings list and positions
                    temp_index[partial_id] = {term: postings}
                    temp_pos[partial_id] = {term: positions}
    ...
    def save_partial_index(self, index_output_folder):
        ...
        with open(index_output_folder + index_file_name, "w") as index_file:
            ...
            positions = self.get_index().get_positions()

            for term in terms:
                # NOTE: Object of type set is not JSON serializable
                term_positions = {doc_id: list(positions[term][doc_id]) for doc_id in positions[term]}

                term_json = {"term": term, "postings": terms[term], "positions": term_positions}
        ...
        if "tfidf" in self.rankings:
            ...
            for term in terms:
                ...
                if self.tfidf_cache:
                    ...
                    term_json = {"term": term, "postings": new_terms[term], "positions": positions[term]}
                    cache_file.write(json.dumps(term_json) + "\n")

                term_json = {"term": term, "postings": terms[term], "positions": positions[term]}
        ...
        for file in files:
            ...
            with open(path_to_folder + "/Final_Index/" + file, "r") as index_file:
                for line in index_file:
                    ...
                    # Convert positions back to a set
                    term_positions = {doc_id: set(positions) for doc_id, positions in index_entry["positions"].items()}

                    # Add term to index
                    index.add_term(index_entry["term"], index_entry["postings"], term_positions)
...
class InvertedIndex(BaseIndex):
    ...
    def add_term(self, term, postings, positions):
        """
        ...
        if term not in self.terms:
            ...
            self.positions[term] = positions
        else:
            self.terms[term].update(postings)

            for doc_id in positions.keys():
                if doc_id not in self.positions[term]:
                    self.positions[term][doc_id] = positions[doc_id]
                else:
                    self.positions[term][doc_id].update(positions[doc_id])
